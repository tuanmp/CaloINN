...
class BaseWriter(L.pytorch.callbacks.BasePredictionWriter):
    ...
    def save_batch(self, incident_energies, showers, batch_idx: str, dataloader_idx: str):
        ...
        save_path = os.path.join(self.batch_dir, "batches", dataloader_idx, f"batch_{batch_idx}.npz")
...
class PredictionWriter(BaseWriter):
    ...
    def write_on_epoch_end(self, trainer: L.Trainer, pl_module: L.LightningModule, predictions, batch_indices):
            ...
            with h5py.File(os.path.join(self.save_dir, self.predict_output_file), "w") as f:
                ...
                for batch_file in batch_files:
                    ...
                    if num_predictions == 0:
                        f.create_dataset("incident_energies", data=incident_energies, maxshape=(None,), chunks=True)
